...
variable "restrict_ips_function_role_name" {
  description = "IP 制限を行う Lambda@Edge 関数に設定する IAM ロール名"
  default     = "restrict-ips-function-role"
}

variable "restrict_ips_function_role_tags" {
  type        = map(string)
  description = "IP 制限を行う Lambda@Edge 関数に設定する IAM ロールのタグ"
  default     = null
}
...
locals {
  restrict_ips_function_role_tags = var.restrict_ips_function_role_tags != null ? var.restrict_ips_function_role_tags : var.restrict_ips_function_tags
}
